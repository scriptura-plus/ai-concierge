...
function getVerseTone(title: string): Tone {
  const torah = new Set([
    'Genesis',
    'Exodus',
    'Leviticus',
...
export default async function ModeratorVerseSelectPage({ params }: PageProps) {
  ...
  if (!selectedBook || !Number.isInteger(numericChapter) || numericChapter < 1) {
    notFound()
  }
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#F7F5EF_0%,#F3F0E8_46%,#F6F3EC_100%)] px-4 py-6">
      <div className="mx-auto flex w-full max-w-md flex-col">
        <Link
          href={`/moderator/workspace/${selectedBook.id}`}
        ...
        <p className="mb-6 mt-2 text-base text-stone-600">Глава {numericChapter}</p>

        <div className="grid grid-cols-4 gap-3">
          {verses.map((verse) => (
            <Link
              key={verse}
              href={`/moderator/workspace/${selectedBook.id}/${numericChapter}/${verse}`}
