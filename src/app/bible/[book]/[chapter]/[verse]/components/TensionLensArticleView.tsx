...
type TensionLensArticleViewProps = {
  article: TensionLensArticle | null
  isLoading: boolean
  error: string
  articleLabel: string
  loadingLabel: string
...
export default function TensionLensArticleView({
  article,
  isLoading,
  error,
  articleLabel,
      ...
      <div className="rounded-[28px] border border-stone-400/20 bg-[radial-gradient(circle_at_top,#fbf5e8_0%,#f2e7cf_55%,#ead9b6_100%)] px-6 py-7 shadow-inner">
        ...
        <div className="mt-6 space-y-5">
          {article.body.map((paragraph, index) => (
            <p
              ...
              className="text-[1.01rem] leading-8 text-stone-800"
            >
              {paragraph}
            </p>
          ))}
        ...
        {article.highlight_line ? (
          <div className="mt-6 rounded-[20px] border border-stone-300/60 bg-[#fffaf1] px-4 py-4">
            <p className="text-[1rem] font-medium leading-8 text-stone-900">
              {article.highlight_line}
            </p>
          </div>
        ) : null}
