  ...
  nodes: [
    {
      ...
      phrase_line:
        'The phrase does not merely mention life; it defines it with unusual precision.',
      what_is_precise:
        'The wording presents eternal life not as duration alone but as something defined in a particular way.',
      why_this_wording:
        'This exact phrasing directs the reader to definition, not only promise.',
      dig_deeper:
        'Trace why the sentence is built as a definition rather than as a reward statement.',
    },
  ],
}
...
function buildPhraseLensCandidatePrompt(params: {
  reference: string
  verseText: string
  ...
  return `
Ты превращаешь результаты Phrase Lens в кандидаты карточек для moderator review в Scriptura+.
...
ЗАДАЧА:
На основе этих phrasing-узлов создай 3-5 сильных candidate-карточек на РУССКОМ языке.
Это должны быть уже хорошие карточки для review, а не сырые заметки.

ГЛАВНЫЙ ПРИНЦИП:
- Каждая карточка должна рождаться из одного phrasing-узла.
...
- быть привязана к одному phrasing-узлу
- показывать, почему эта формулировка реально меняет чтение стиха
...
async function buildRussianCandidatesFromNodes(params: {
  ...
  const prompt = buildPhraseLensCandidatePrompt(params)
  ...
  if (!rawText) {
    throw new Error('Phrase Lens candidate generator returned no usable text.')
  }
  ...
  if (!options || options.length === 0) {
    throw new Error('Failed to parse Phrase Lens candidate cards.')
  }
    ...
    if (mode === 'map') {
      ...
      try {
        ...
        if (parsedRef) {
          ...
          if (options.length > 0) {
            ...
            if (freshItems.length > 0) {
              ...
              if (error) {
                throw new Error(`Failed to save Phrase Lens candidates: ${error.message}`)
              }
        ...
        candidateIntakeError =
          error instanceof Error ? error.message : 'Phrase Lens candidate intake failed.'
      }
