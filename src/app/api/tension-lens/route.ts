  ...
  nodes: [
    {
      ...
      tension_line:
        'The verse sounds stable on the surface, but the logic turns in an unexpected direction.',
      what_feels_strange:
        'It joins two ideas that a modern reader normally keeps apart.',
      why_it_matters: 'This tension changes how the verse should be heard.',
      dig_deeper:
        'Trace why the wording forces that collision instead of a smoother statement.',
    },
  ],
}
...
function buildTensionLensCandidatePrompt(params: {
  reference: string
  verseText: string
  ...
  return `
Ты превращаешь результаты Tension Lens в кандидаты карточек для moderator review в Scriptura+.
...
ЯЗЫК ИСХОДНОЙ TENSION LENS MAP:
...
ЗАДАЧА:
На основе этих tension-узлов создай 3-5 сильных candidate-карточек на РУССКОМ языке.
Это должны быть уже хорошие карточки для review, а не сырые заметки.

ГЛАВНЫЙ ПРИНЦИП:
- Каждая карточка должна рождаться из одного tension-узла.
- Не делай общую проповедь по стиху.
- Не смешивай всё в один расплывчатый комментарий.
- Карточка должна держать один чёткий угол напряжения.
...
- быть привязана к одному tension-узлу
- показывать, почему это напряжение реально меняет чтение стиха
...
async function buildRussianCandidatesFromNodes(params: {
  ...
  const prompt = buildTensionLensCandidatePrompt(params)
  ...
  if (!rawText) {
    throw new Error('Tension Lens candidate generator returned no usable text.')
  }
  ...
  if (!options || options.length === 0) {
    throw new Error('Failed to parse Tension Lens candidate cards.')
  }
    ...
    if (mode === 'map') {
      ...
      try {
        ...
        if (parsedRef) {
          ...
          if (options.length > 0) {
            ...
            if (freshItems.length > 0) {
              ...
              if (error) {
                throw new Error(`Failed to save Tension Lens candidates: ${error.message}`)
              }
        ...
        candidateIntakeError =
          error instanceof Error ? error.message : 'Tension Lens candidate intake failed.'
      }
